...
> **기준**: xAI 공식 문서([docs.x.ai](https://docs.x.ai)) 기준 모델 라인업, 티어별 호출 한도(Rate Limits), 주요 용도 및 그라운딩 도구 요약 (2026년 9월 업데이트)
...
| :--- | :--- | :--- |
| **RPS** | **R**equests **P**er **S**econd | **초당 보낼 수 있는 최대 API 호출 수**입니다. 순간적인 트래픽 폭주(Burst)를 방지하기 위해 제한합니다. *(예: 3 RPS = 1초에 최대 3번 요청 가능)* |
| **RPM** | **R**equests **P**er **M**inute | **1분 동안 보낼 수 있는 최대 API 호출 수**입니다. *(RPM ÷ 60 = RPS)* |
| **TPM** | **T**okens **P**er **M**inute | **1분 동안 처리 가능한 토큰(글자/단어 단위) 총량**입니다. (질문 입력 + 모델 답변 + Reasoning 사고 토큰 + 캐시 토큰 모두 합산) |
| **RPD** | **R**equests **P**er **D**ay | **하루(24시간) 동안 API를 호출할 수 있는 최대 횟수**입니다. |
| **Context Window** | 컨텍스트 윈도우 | **모델이 한 번에 기억하고 처리할 수 있는 최대 문맥 크기**입니다. 대화 기록, 첨부 코드/문서, 모델 답변이 모두 이 안에 들어가야 합니다. |
| **단위 (K / M)** | Kilo / Million | **K = 1,000 (천)** / **M = 1,000,000 (백만)** <br> *(예: 128K = 약 12만 8천 토큰, 10M = 1,000만 토큰)* |
...
| :---: | :--- | :--- | :---: | :---: | :---: | :--- |
| 1 | **Grok 4.6** (`grok-4.6`) | Flagship (주력) | 최대 500K | 3 | 10M | xAI 최고 지능의 최신 플래그십 모델. 복합 추론, 심층 코딩, 대규모 문서 분석 및 범용 멀티모달 대화 |
...
| 6 | **Grok 4.1 Fast** (`grok-4.1-fast`) | Fast / Routing | 2M | 5+ | 10M | 초대형 컨텍스트 지원. 초저지연·고처리량 지원 챗봇, 실시간 데이터 라우팅 및 1차 응답 분류 모델 |
| 7 | **Grok 4.3** (`grok-4.3`) | Real-time Search | 1M | 3 | 10M | 실시간 X(구 Twitter) 데이터 검색 및 소셜 트렌드 분석 연계에 최적화된 모델 |
...
xAI는 **2026년 1월 1일 이후 누적 결제 금액(Cumulative Spend)**에 따라 분당/초당 요청 한도를 영구적으로 상향하는 티어 시스템을 사용합니다.
...
| **Tier 4** | $5,000 이상 | 500 RPS | 100M+ TPM | 대규모 동시 요청 및 배치 처리 |
...
| **MCP (Model Context Protocol)** | Grok 4.x, Grok 3 계열 | 기본 기능 (서버 연동) | 외부 서드파티 툴 및 로컬 환경과 원활히 연결할 수 있는 표준 프로토콜 지원 |
...
- **입출력 토큰 과금 (예시 기준)**:
  - **Grok 4.5**: 입력 $2.00 / 1M 토큰, 출력 $6.00 / 1M 토큰
  - **Grok 4.3**: 입력 $1.25 / 1M 토큰, 출력 $2.50 / 1M 토큰
  - **Grok 4.20 (Non-Reasoning)**: 출력 $2.50 / 1M 토큰
